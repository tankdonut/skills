...
| [`update-opencode-plugins`](skills/update-opencode-plugins/) | Bump `name@version` entries in `opencode.json` to the latest npm release, with an optional 7-day cooldown so too-fresh versions are skipped, clear the stale plugin cache at `~/.cache/opencode/packages`, and pre-install each bumped version into that cache with a fully controlled `NPM_CONFIG_*` environment (ambient npmrc settings stripped) so opencode loads exactly the pinned version via its cache fast path. |
